...
import { Users, UserMinus, User, XCircle, ChevronDown } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // const response = await fetch('/api/manager/team-dashboard');
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6 text-black bg-[#E5E7EB] min-h-full">
      ...
      <div className="bg-[#0B0F4E] rounded-2xl p-8 flex flex-col md:flex-row items-start md:items-center justify-between text-white relative overflow-hidden shadow-md">
        <div className="z-10">
          ...
          <p className="text-[#9EA1FF] text-sm font-medium">ยินดีต้อนรับสู่ Dashboard ของแผนกคุณ</p>
        </div>
        ...
        <div className="absolute bottom-4 right-6 z-10">
          <Link href="/dashboard/manager" className="text-xs text-[#9EA1FF] hover:text-white underline-offset-4 hover:underline font-medium">
            ดู Dashboard ของคุณ
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <Card className="rounded-2xl border-none shadow-sm hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            ...
            <p className="text-[13px] font-bold text-gray-600 mb-1">พนักงานทั้งหมด</p>
            <div className="flex items-baseline gap-2">
              <span className="text-[40px] font-bold text-gray-800 leading-none">{data.stats.totalEmployees}</span>
              <span className="text-sm font-semibold text-gray-500">คน</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-none shadow-sm hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            <div className="w-10 h-10 rounded-xl bg-purple-100 flex items-center justify-center mb-6">
              <UserMinus className="w-5 h-5 text-purple-600" />
            </div>
            <p className="text-[13px] font-bold text-gray-600 mb-1">จำนวนการลาวันนี้</p>
            <div className="flex items-baseline gap-2">
              <span className="text-[40px] font-bold text-gray-800 leading-none">{data.stats.leavesToday}</span>
              <span className="text-sm font-semibold text-gray-500">คน</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-none shadow-sm hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            ...
            <p className="text-[13px] font-bold text-gray-600 mb-1">พนักงานคงเหลือ</p>
            <div className="flex items-baseline gap-2">
              <span className="text-[40px] font-bold text-gray-800 leading-none">{data.stats.remainingEmployees}</span>
              <span className="text-sm font-semibold text-gray-500">คน</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-none shadow-sm hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            ...
            <p className="text-[13px] font-bold text-gray-600 mb-1">โควต้าการลาในวันนี้</p>
            <div className="flex items-baseline gap-2">
              <span className="text-[40px] font-bold text-gray-800 leading-none">{data.stats.quotaToday}</span>
              <span className="text-sm font-semibold text-gray-500">สิทธิ์</span>
            </div>
          </CardContent>
        </Card>
      </div>
